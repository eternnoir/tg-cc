...
type BotConfig struct {
	...
	WorkingDir string
	Whitelist  []int64
	ClaudeArgs []string // Additional arguments to pass to Claude CLI
	TempDir    string   // Temporary directory for file uploads
}
...
func loadBotFromEnv(prefix string) (BotConfig, error) {
	...
	claudeArgsStr := os.Getenv(prefix + "CLAUDE_ARGS")
	if claudeArgsStr != "" {
		bot.ClaudeArgs = parseClaudeArgs(claudeArgsStr)
	}

	// Get temp directory for file uploads
	bot.TempDir = os.Getenv(prefix + "TEMP_DIR")
	if bot.TempDir != "" {
		// Create temp directory if it doesn't exist
		if err := os.MkdirAll(bot.TempDir, 0755); err != nil {
			return bot, fmt.Errorf("failed to create temp directory %s: %w", bot.TempDir, err)
		}
	}
...
// parseClaudeArgs parses a string of Claude CLI arguments
// Supports both space-separated and comma-separated formats
// Examples:
//   - "--model claude-sonnet-4-20250514 --max-turns 10"
//   - "--allowedTools,Bash,Read,Write"
func parseClaudeArgs(argsStr string) []string {
